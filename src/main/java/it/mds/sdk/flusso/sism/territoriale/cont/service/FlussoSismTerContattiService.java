package it.mds.sdk.flusso.sism.territoriale.cont.service;
...
import it.mds.sdk.flusso.sism.territoriale.cont.parser.regole.ParserTracciatoImpl;
...
import it.mds.sdk.flusso.sism.territoriale.cont.tracciato.TracciatoSplitterImpl;
...
@Slf4j
@Component("flussoSismTerContService")
public class FlussoSismTerContattiService {


    private ValidationEngine validationEngine;
    private TracciatoSplitter<RecordDtoSismTerritorialeContatti> tracciatoSplitter;
    private ConnettoreMds connettoreMds;
    private final ConfigurazioneFlussoSismTerContatti conf;

    private static final String CATEGORIA_FLUSSI = "TER";
    ...
    private static final String XML_FLUSSO_TAG = "</TerritorialeContatto><TerritorialeContatto>";
    private static final String NOME_FILE_XSD_SISM = "CNT.xsd";

    @Autowired
    public FlussoSismTerContattiService(@Qualifier("validationEngine") final ValidationEngine validationEngine,
                                        @Qualifier("tracciatoSplitterSismTerCont") final TracciatoSplitter<RecordDtoSismTerritorialeContatti> tracciatoSplitter,
                                        @Qualifier("connettoreMds") final ConnettoreMds connettoreMds,
                                        @Qualifier("configurazioneFlussoSismTerCont") ConfigurazioneFlussoSismTerContatti conf) {
    ...
    public void validazioneBlocchi(int dimensioneBlocco, String nomeFile, RegoleFlusso regoleFlusso, String idRun,
        ...
        TerritorialeContatto territorialeContatto = null;
        File file = getfileFromPath(conf.getFlusso().getPercorso() + nomeFile);
        String nomeFileXml = conf.getXmlOutput().getPercorso() + "SDK_TER_CNT_" + periodoRiferimento + "_" + idRun + ".xml";
        ...
        do {
            ...
            List<RecordDtoSismTerritorialeContatti> recordConv =
                    bloccoValidazione.getRecordList().stream().map(RecordDtoSismTerritorialeContatti.class::cast).collect(Collectors.toList());
            ...
            if (!recordConv.isEmpty()) {
                ...
                territorialeContatto = ((TracciatoSplitterImpl) tracciatoSplitter).creaTerritorialeContatti(recordConv, territorialeContatto);
            } else {
            ...
            if (isWriteOk) {
                ...
                try {
                    gestoreFile.scriviDtoFragment(territorialeContatto, nomeFileXmlTmp, urlXsd);
